...
export default {
  ...
  computed: {
    ...
    payMoney: function() {
      if (this.$store.state.total > 28)
        return this.$store.state.total - 5 + this.seller.deliveryPrice;
      else
        return this.$store.state.total + this.seller.deliveryPrice;
    }
  },
  methods: {
    goBack: function() {
    ...
    submitOrder: function() {
      ...
      this.$http.submitOrder(JSON.stringify(order))
        .then(res => {
          if (res.data.status) {
            this.$message({
